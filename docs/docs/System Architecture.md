...
|---|---|---|
| `GET /auth/google` | No | Starts Google OAuth flow. Redirects to Google consent page. |
| `GET /auth/google/callback` | No | OAuth callback. Sets session cookie, redirects to frontend. |
| `GET /auth/logout` | No | Destroys the current session if one exists. Returns `{ success: true }`. |
| `GET /auth/user` | Yes | Returns current user object or `401` if not authenticated. |

### Todo Endpoints 

| Method + Path | Description |
|---|---|
| `GET /items` | Returns todos for the authenticated user.|
| `POST /items` | Creates a todo. Returns created `Todo` object. |
| `PUT /items/:id` | Updates a todo. Returns updated `Todo`. |
| `DELETE /items/:id` | Deletes a todo and all its category associations. Returns HTTP `200`. |
| `PUT /items/:id/priority` | Updates priority only. Returns updated `Todo`. |

### Category Endpoints 

| Method + Path | Description |
|---|---|
| `GET /categories` | Returns all categories (global, not user-scoped). |
| `POST /categories` | Creates a category.|
| `POST /items/:itemId/categories` | Links a category to a todo. Returns updated `Todo`. |
| `DELETE /items/:itemId/categories` | Unlinks a category from a todo. Returns updated `Todo`. |

---

## Deployment Infrastructure

The application is deployed on AWS EC2 and managed via Docker Compose. 
- The EC2 instance must have Docker and Docker Compose installed.
- Port `8080` must be open in the EC2 Security Group for inbound TCP traffic.
- The `.env` file on the server is written automatically by GitHub Actions on every deployment and is never committed to the repository.
- The Docker named volume `todo-db` persists across deployments, so data isn't lost when containers are rebuilt.
- All five containers communicate exclusively over the internal `todo-network` bridge.
- `CLIENT_ORIGIN` and `BACKEND_URL` are hardcoded to `http://ec2-18-226-88-223.us-east-2.compute.amazonaws.com:8080` in `docker-compose.yml`. 
